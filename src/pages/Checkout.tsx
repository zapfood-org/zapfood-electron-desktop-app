...
// Tipos locais estendidos para a UI do checkout
interface CheckoutOrderItem extends OrderItemDTO {
  name: string;
  isPaid: boolean;
  paidQuantity: number;
}
...
export function CheckoutPage() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      try {
        ...
        const apiOrder: Order = response.data;

        // Fetch product names for all items
        const productPromises = apiOrder.items.map(async (item: OrderItem) => {
          try {
            ...
            return {
              itemId: item.id,
              name: productResponse.data.name || "Produto sem nome",
            };
          } catch (error) {
        ...
        const productNameMap = new Map(
          productNames.map((p) => [p.itemId, p.name])
        );

        // Transform Items to CheckoutOrderItem
        const items: CheckoutOrderItem[] = (apiOrder.items || []).map(
          (i: OrderItem, index: number) => ({
            id: i.id || `item-${index}`,
            ...
            name: productNameMap.get(i.id) || "Produto sem nome",
            isPaid: false,
            paidQuantity: 0,
          })
        );
        ...
        let tableName = "";
        // Check if order has tableId (might be in API response but not in DTO type)
        const apiOrderWithTable = apiOrder as Order & { tableId?: string };
        const tableId = apiOrderWithTable.tableId;
        if (tableId) {
          try {
        ...
        const fetchedOrder: CheckoutOrder = {
          id: apiOrder.id,
          table: tableName || undefined,
          customer: apiOrder.customerName || undefined,
          items: items,
          subtotal: subtotal,
  ...
  const getUnpaidQuantity = (item: CheckoutOrderItem) => {
  ...
  const getSelectedQuantity = (item: CheckoutOrderItem) => {
    if (expandedItems.has(item.id)) {
      ...
      return selectedItems.has(item.id) && !item.isPaid
        ? getUnpaidQuantity(item)
        : 0;
    }
  };
  ...
  const remainingTotal = splitAmount
    ? splitAmount
    : hasSelection
    ? selectedTotal
    : unpaidTotal;
  ...
  const paymentMethods = [
    {
      id: "credit",
      label: "Crédito",
      icon: <CardIcon size={32} weight="Bold" />,
    },
  ...
  const handleKeypadPressCallback = useCallback(
    (key: string) => {
      const MAX_DIGITS = 10;
      const newValueString = amountPaidString + key;
  ...
  const handleFinish = async () => {
    ...
    if (allItemsPaid) {
      ...
      try {
        ...
        toast.error(
          "Erro ao finalizar o pedido. O pagamento foi registrado localmente."
        );
      ...
      toast.success(
        `${paidUnitsCount} ${
          paidUnitsCount === 1 ? "unidade foi paga" : "unidades foram pagas"
        } com sucesso!`
      );
    }
  };
        ...
        <div className="p-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold">
              {order?.table ? `Mesa ${order.table}` : "Pedido"}
            </h1>
          ...
          <div className="flex flex-col gap-3 px-6 py-4">
            {orderItems.map((item) => {
              ...
              const isItemSelected =
                !isExpanded && selectedItems.has(item.id) && !isPaid;
                      ...
                      <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {isPartiallyPaid && (
                              <span className="text-xs text-warning font-semibold bg-warning/10 px-2 py-0.5 rounded-full">
                                {item.paidQuantity}/{item.quantity} pago
                                {item.paidQuantity! > 1 ? "s" : ""}
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        {canExpand ? (
                          ...
                          <Button
                            ...
                            variant="light"
                            disabled
                            className="flex-shrink-0 text-default-100"
                          >
                      ...
                      {Array.from({ length: item.quantity }, (_, index) => {
                        ...
                        return (
                          <Card
                            ...
                            onPress={() =>
                              !unitIsPaid && handleToggleUnit(item.id, index)
                            }
                          >
                                ...
                                <div className="flex-shrink-0">
                                  {!unitIsPaid ? (
                                    <Checkbox
                                      ...
                                      onValueChange={() =>
                                        handleToggleUnit(item.id, index)
                                      }
                                      size="md"
                                      color="primary"
                        ...
                        {selectedPayment === "cash" ? (
                          <>
                            <div className="flex flex-col w-full gap-4">
                              ...
                              <div className="flex justify-between items-center bg-white dark:bg-default-50 p-4 rounded-xl border border-default-200 dark:border-default-100 border-transparent">
                                <span className="text-default-500 dark:text-default-400 font-medium">
                                  Troco
                                </span>
                                <span
                          ...
                          <div className="flex flex-col items-center justify-center h-full text-center py-12">
                            ...
                            <p className="text-default-400 text-sm">
                              Selecione "Dinheiro" para acessar o teclado
                              numérico
                            </p>
                          </div>
                        )}
