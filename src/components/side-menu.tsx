...
export function SideMenu() {
  ...
  const filteredMenuItems = useMemo(() => {
    const items = menuItems((tenantId as string) ?? "1").filter(
  ...
  return (
    <aside
      className={clsx(
        "flex flex-col bg-default-50 border-r border-default-200 transition-all duration-300",
        isOpen ? "w-80" : "w-20"
      )}
    >
      {/* Header */}
      ...
      <div className="p-4 space-y-4">
        ...
        {isOpen ? (
          <Button
            ...
            startContent={
              mounted && theme === "light" ? (
                <Sun size={20} weight="Outline" />
              ) : (
                <Moon size={20} weight="Outline" />
              )
            }
          >
