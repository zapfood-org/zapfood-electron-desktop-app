...
import type { OrderItem } from "../../dtos/OrderDTO";
...
export interface Order {
  ...
  address: string;
  total: number;
  deliveryType: "delivery" | "dine_in" | "pickup";
...
export function OrderCard({
  ...
  const isVeryUrgent =
    isPending && moment().diff(order.createdAt, "minutes") > 25;
  ...
  return (
    <Card
      className={`w-full border border-default-200 transition-all ${
        isUrgent ? "border border-warning" : ""
      } ${isVeryUrgent ? "border-danger bg-danger-50" : ""}`}
    >
                    ...
                    <Button
                      ...
                      startContent={
                        order.deliveryType === "dine_in" ? (
                          <BillList size={18} weight="Outline" />
                        ) : (
                          <CheckCircle size={18} weight="Outline" />
                        )
                      }
                      onPress={() => {
