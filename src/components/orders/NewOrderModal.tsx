  ...
  useEffect(() => {
    if (orderToEdit && isOpen) {
      ...
      if (orderToEdit.items && orderToEdit.items.length > 0) {
        ...
        orderToEdit.items.forEach(
          (item: {
            ...
            const product =
              ...
              products.find((p) => p.name === prodName);
        ...
        // Use observation directly from order if available, otherwise parse from description
        observation =
          orderToEdit.observation ||
          (orderToEdit.description
            ? orderToEdit.description.split(" - ").slice(1).join(" - ")
            : "");
      } else {
        ...
        if (
          orderToEdit.deliveryType === "pickup" ||
          orderToEdit.deliveryType === "dine_in"
        ) {
      ...
      setFormData({
        ...
        customerPhone:
          orderToEdit.customerPhone === "Não informado"
            ? ""
            : orderToEdit.customerPhone,
    ...
    if (formData.table && formData.bill) {
      try {
        ...
        const existingBillIds =
          tableData.bills?.map((b: { id: string }) => b.id) || [];

        // Merge with new bill ID
        const updatedBillIds = Array.from(
          new Set([...existingBillIds, formData.bill])
        );
              ...
              {(formData.deliveryType === "pickup" ||
                ...
                <div className="flex flex-col gap-4">
                  <Select
                    ...
                    selectedKeys={
                      formData.table ? new Set([formData.table]) : new Set([])
                    }
                  ...
                  <Select
                    ...
                    selectedKeys={
                      formData.bill ? new Set([formData.bill]) : new Set([])
                    }
                    disabledKeys={bills
                      ...
                      >
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          <Button
                            ...
                            className="min-w-8 h-8"
                            onPress={() =>
                              handleRemoveProduct(orderProduct.product.id)
                            }
                          >
